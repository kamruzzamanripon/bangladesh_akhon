    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. __( 'Error! Can not load json file.', 'advance-wordpress' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. __( 'Add Icon', 'advance-wordpress' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. __( 'Search a Icon...', 'advance-wordpress' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. __( 'Loading...', 'advance-wordpress' ) .'</div></div>';
    echo '</div>';
